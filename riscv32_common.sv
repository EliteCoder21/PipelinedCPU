...
function automatic ext_operand execute(
    ...
    case (op_q)
        ...
        q_jal:              result = {1'b0, pc} + 32'd4;
        q_jalr:             result = {1'b0, pc} + 32'd4;
        q_branch:           result = 0;
...
function automatic word compute_next_pc(
    ext_operand rs1_val,      // cast_to_ext_operand(rd1)
    ext_operand exec_result,  // ALU result (rs1 - rs2 for branches typically)
    word        imm,
    word        pc,
    opcode_q    op_q,
    funct3      f3
    ...
    case (op_q)
        ...
        q_branch: begin
            ...
            case (f3)
                3'b000: take_branch = (exec_result == 0);  // BEQ
                3'b001: take_branch = (exec_result != 0);  // BNE
                3'b100: take_branch = ($signed(exec_result) < 0);  // BLT
                3'b101: take_branch = ($signed(exec_result) >= 0); // BGE
                3'b110: take_branch = (exec_result[31] == 1'b1);   // BLTU (borrow)
                3'b111: take_branch = (exec_result[31] == 1'b0);   // BGEU
        ...
        // -----------------------------
        q_jalr: begin
            next_pc = word'(rs1_val + imm); // & 32'hFFFF_FFFE;
        end
